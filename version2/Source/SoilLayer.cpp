...
Layer(width, upperLeftCorner) {
    int i;
    SDL_Point pos;
    std::array<SimpleTextureEnum, 4> grassTypes, plants;
    ...
    for (i = 0; i <= 3; ++i) {
        posGrass = {upperLeftCorner.x + i*SimpleTexture::get_width(), upperLeftCorner.y};
        posPlant = {upperLeftCorner.x + i*SimpleTexture::get_width(), 
                    upperLeftCorner.y + SimpleTexture::get_height()};
